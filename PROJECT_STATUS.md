...
- The homepage hero uses one subdued, partially cropped upper-left botanical ornament in paired Ivory Ink and Marine Ink palettes. The name is deliberately set on exactly three rows (`Martyna`, `Katarzyna`, `Zwoińska`) at every width. Desktop and wide tablet layouts (760 px and above) retain the current organic emerald-and-gold portrait frame; the 621–759 px compact layout omits the portrait but retains the upper flourish. A single small antique-gold diamond closes the non-phone hero without horizontal rules. Phones omit the portrait, ornament and diamond, centre the three-line name, and use tighter vertical spacing before About. The localized role remains an identity line (`Evolutionary biologist`) and one expertise line (`Experimental research · Genomics · Data analysis · AI tools`); at 759 px and below, the expertise is grouped into two balanced lines without a dangling separator. On phones, the affiliation stacks the department, university and country on separate lines; wider layouts retain comma-separated wording. The standard divider remains suppressed only above About, while the full gold line and diamond between Research and the project subsection remain unchanged.
- The approved hero sources are `photo.jpg`, `assets/hero/quiet-atelier-upper-left.png`, `assets/hero/quiet-atelier-upper-left-dark.png`, `assets/hero/quiet-atelier-lower-right.png`, and `assets/hero/quiet-atelier-lower-right-dark.png`. The lower-right pair is retained as source material but is not displayed. Preserve these assets rather than reconstructing their ornament. Display headings use real Cormorant Garamond 700 with a subtle theme-aware optical stroke. The hero name uses `.46px` in Ivory Ink and `.34px` in Marine Ink; do not substitute a synthetic 800 weight. At every compact breakpoint, the expertise is divided into the same two balanced phrases: `Experimental research · Genomics` and `Data analysis · AI tools`. The narrow-phone hero uses a restrained 20 px bottom pad and 12 px affiliation margin.
