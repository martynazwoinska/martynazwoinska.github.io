...
- The homepage hero leads with the two-line statement `From lab to code, from ideas to execution`. On desktop and tablet from 760 px upward, the full name follows as a larger Cormorant Garamond identity line, with two compact sans-serif expertise lines beneath it: `Evolutionary biology · Experimental research` and `Genomics · Data analysis · Applied AI`. The statement, name and expertise share one editorial left edge; the complete portrait-and-copy group is positioned as one unit rather than centring individual lines. The institutional affiliation is intentionally omitted from the hero because it is available in Contact and structured metadata. Swedish and Polish use the corresponding approved localized statement and expertise wording. These wider layouts use a fuller, optically balanced portrait-and-text composition: the circular `photo.jpg` portrait sits inside a fine organic antique-gold and emerald frame, while one subdued botanical ornament is inset slightly from the lower-right edge to relate more closely to the central composition. A central antique-gold diamond with two short hairlines closes the hero. Below 760 px, the portrait, statement and gold pause are omitted; the identity-led lockup places the reinforced name first on exactly three lines. From 621 to 759 px the expertise forms a two-by-two visual grid with a larger ornament integrated at the right. At 620 px and below, the centred expertise forms a three-step inverted pyramid: `Evolutionary biology · Experimental research`, then `Genomics · Data analysis`, then `Applied AI`; at 350 px and below the first pair separates for a safe four-row lockup. The enlarged ornament follows in normal document flow and completes the lower-right corner without colliding with translated copy. The standard divider remains suppressed only above About. The full-width antique-gold rule between Research and the project subsection uses the same six-pixel diamond and 20-pixel central breathing gap as the hero pause; only the rule length differs.
...
1. Revise the remaining 16 Game locations one at a time. Validate and update the existing dossier where one is already present; create a source-backed dossier wherever one still lacks one before revising its landscape and three accessory pairs.
