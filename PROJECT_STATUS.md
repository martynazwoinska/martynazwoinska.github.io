...
- The homepage hero leads with the two-line statement `From lab to code, from ideas to execution` and follows it with two compact expertise lines: `Evolutionary biology · Experimental research` and `Genomics · Data analysis · Applied AI`. The full name appears as a slightly larger standalone supporting line beneath the expertise; the institutional affiliation is intentionally omitted from the hero because it is available in Contact and structured metadata. Swedish and Polish use the corresponding approved localized statement and expertise wording. Desktop and tablet layouts use a fuller, optically left-shifted portrait-and-text composition: the circular `photo.jpg` portrait sits inside a fine organic antique-gold and emerald frame, while one subdued botanical ornament remains anchored at the lower right. A central antique-gold diamond with two short hairlines closes the hero. At 759 px and below, the portrait and gold pause are omitted; the same ornament is reduced and follows the centred text in normal flow so translated copy cannot collide with it. The standard divider remains suppressed only above About, while the full gold line and diamond between Research and the project subsection remain unchanged.
