...
- The approved hero sources are `photo.jpg`, `assets/hero/quiet-atelier-upper-left.png`, `assets/hero/quiet-atelier-upper-left-dark.png`, `assets/hero/quiet-atelier-lower-right.png`, and `assets/hero/quiet-atelier-lower-right-dark.png`. Preserve these assets rather than reconstructing their ornament. The hero name uses Cormorant Garamond at weight 700 with a subtle `.28px` current-colour optical stroke; do not substitute a synthetic 800 weight.
- Shared theme and language-preference handling is present under `shared/`.
- The homepage footer uses a quiet 34% surface / 66% page-background mix with its antique-gold top rule.
