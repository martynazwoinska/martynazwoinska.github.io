...
- The homepage hero leads with the two-line statement `From lab to code, from ideas to execution.` and follows it with two compact expertise lines: `Evolutionary biology · Experimental research` and `Genomics · Data analysis · Applied AI`. The full name and institutional affiliation remain present in a quieter supporting treatment. Swedish and Polish use the corresponding approved localized statement and expertise wording. Desktop and tablet layouts place the circular `photo.jpg` portrait on the left inside a fine organic antique-gold and emerald frame, with one subdued lower-right botanical ornament and a small central antique-gold diamond. At 759 px and below, the portrait and diamond are omitted; the same ornament is reduced and follows the centred text in normal flow so translated copy cannot collide with it. The standard divider remains suppressed only above About, while the full gold line and diamond between Research and the project subsection remain unchanged.
