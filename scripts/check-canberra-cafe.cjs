...
class Matrix {inverse(){return this;}multiply(){return this;}translate(){return this;}rotate(){return this;}toString(){return 'matrix(1 0 0 1 0 0)';}}
...
(async()=>{
  ...
  for(const kind of ['sip','bite','raid','wipe'])for(const reduced of [false,true]) {
    ...
    assert(cafeFrame(5000,kind,reduced).done);assert(cafeFrame(reduced?280:kind==='raid'?1050:kind==='wipe'?750:1000,kind,reduced).contact);
  }
  ...
  const coffee=makePiece('canberra-flat-white-cafe','primary',drawCafeProps);
  assert(coffee.querySelector('[data-cafe-cup]'),'Dedicated primary cup renderer');
  assert(coffee.querySelector('[fill-rule="evenodd"]'),'Open ceramic handle');
  ...
  assert(ctl.start(bird));step(1050);assert.equal(last.dataset.consumed,'stolen');step(1800);assert.equal(last.getAttribute('opacity'),'0');assert.equal(root.querySelectorAll('[data-cafe-overlay]').length,0);
