...
(async()=>{
 const {sketchFrame,sketchPose,boundedPeek,DURATION}=await import(pathToFileURL(path.join(__dirname,'../game-of-worms/orsay-play.js')));
 ...
 for(let t=10;t<=DURATION+200;t+=10){const f=sketchFrame(t);for(const key of ['enter','leave','open','progress','peek','envelope']){assert(f[key]>=0&&f[key]<=1);assert(Math.abs(f[key]-last[key])<.04,`No jump in ${key}`);}last=f;}
