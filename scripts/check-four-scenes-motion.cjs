...
(async()=>{
 ...
 const {feedingFrame,FEEDING_DURATION,createN2Feeding}=await mod('n2-feeding.js');
 ...
 for(let ms=0;ms<=FEEDING_DURATION;ms+=10){
  ...
  if(ms<2300)assert.equal(f.consumed,false,'Interrupted reaching does not consume a portion');
