...
- At wide viewports, treat the painted botanical frame as a pair of viewport-fixed growths rather than page-bound decoration: the upper-left piece hangs from the current top edge, leaves a deliberate clear gap before the `MZ` monogram and the lower-right piece rises from the current bottom edge. Build both theme derivatives from the pristine source painting so the foliage and gold retain their original local contrast, but process their alpha contours independently: Petrol Ink keeps the tighter high-contrast extraction, while Ivory Ink preserves a fuller, softer painted edge and uses the warmer fuchsia flower treatment. Do not add a page-colour veil or alter the preserved source artwork.
