...
The homepage hero uses `photo.jpg` with the approved lower-right ornament pair under `assets/hero/`: `quiet-atelier-lower-right.png` and `quiet-atelier-lower-right-dark.png`. From 760 px upward, the ornament sits to the lower right of the portrait-and-copy composition; the name is a larger Garamond identity line between the statement and sans-serif expertise. Below 760 px, use an identity-led lockup without the portrait or statement: the name appears first on exactly three lines, followed by the unchanged expertise wording. From 621 to 759 px the expertise forms a two-by-two visual grid and the enlarged ornament is integrated at the right; at 620 px and below the lockup is centred and the ornament follows in normal document flow. The hero pause and the full-width rule before the project subsection share one central detail: a six-pixel antique-gold diamond inside a 20-pixel break in the line. The project rule extends to both content edges, while the hero uses only two short hairlines. The upper-left ornament pair remains preserved as approved source material. Treat all five hero files as source assets and preserve their composition instead of redrawing them from screenshots.
