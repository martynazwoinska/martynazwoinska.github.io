...
The navigation background matches the page background in both themes so the header does not compete with the hero. Fine antique-gold borders separate the header rows and close the header consistently. The 58 × 38 px monogram remains visible in full navigation and in compact navigation from 320 px upward without a separate trailing rule. The 360–419 px header uses a slightly tighter six-pixel column gap; below 360 px, the menu becomes a 44 px icon-only control while its translated label remains available to assistive technology. At 320–359 px, a four-pixel column gap preserves the full-size monogram; only headers below 320 px omit the lockup. The theme toggle keeps a 34 px control area and clean 28 px antique-gold face at every viewport; in Petrol Ink, do not desaturate that face by blending it into the petrol field. The desktop glyph is 20 px; in the compact mobile header, only the half-filled `#14171a` glyph reduces to 17 px while the face keeps its size and half-pixel hairline.
...
- Do not display a viewport-fixed botanical frame at any width. Keep its approved transparent sources and derivatives archived but unused; the responsive hero-local ornament is the only active large botanical decoration.
