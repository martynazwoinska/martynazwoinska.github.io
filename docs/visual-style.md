...
The navigation background matches the page background in both themes so the header does not compete with the hero. Fine antique-gold borders separate the header rows and close the header consistently. The theme toggle keeps a 34 px control area and clean 28 px antique-gold face at every viewport; in Petrol Ink, do not desaturate that face by blending it into the petrol field. The desktop glyph is 20 px; in the compact mobile header, only the half-filled `#14171a` glyph reduces to 17 px while the face keeps its size and half-pixel hairline.
...
- From 1780 px upward, use one deliberately asymmetric viewport-fixed botanical growth at the lower right; do not display an upper-left viewport ornament. The complete growth rises from the current bottom-right edge with 6 px of breathing room and is sized from the available outer gutter, leaving 24 px beside the 1160 px content field. It scales responsively from 280 px to 410 px wide so it remains subordinate to the content, including a clear gutter beside the homepage Research figures. Use exactly the same alpha geometry and positioning in Ivory Ink and Petrol Ink, changing only palette values. Ivory Ink uses warm olive and bottle green, antique gold, warm ivory and berry-fuchsia; Petrol Ink deepens the greens and uses a saturated plum-violet flower while retaining antique-gold and cream details. Preserve the transparent source masters, including the archived upper-left source, and do not add a page-colour veil, rectangular matte, glow or sticker-like outline.
...
The homepage hero uses `photo.jpg`. Its ornament has exactly two responsive states. Below the 1780 px fixed-frame threshold, use the dedicated compact crescent artwork at a responsive scale in desktop, tablet and mobile layouts. From 1780 px upward, suppress that hero-local crescent and use the complete viewport-fixed botanical frame, sized from the available outer gutter at a responsive 280–410 px width. Do not insert an intermediate, cropped or hybrid hero ornament between those states. The former `assets/hero/quiet-atelier-lower-right.png` and `assets/hero/quiet-atelier-lower-right-dark.png` files remain preserved source assets but are not displayed. From 760 px upward, the portrait, copy and compact crescent form one composition: the portrait-to-copy gap is restrained, the ornament is inset from the outer edge, and the reinforced Garamond name sits between the statement and sans-serif expertise. Below 760 px, use one consistent centred identity lockup without the portrait or statement: the reinforced name appears first on exactly three lines and the expertise forms a three-step inverted pyramid. Position the same compact crescent at the far right of that expertise band: one plant, exactly one open flower, one outward bud and a fine lower curl beneath `Applied AI`. It is absolutely positioned within the hero so it wraps the lockup rather than consuming a separate row; it uses one shared reinforced 822 × 741 geometry in both themes so its principal stem remains visibly continuous after mobile downscaling, while Ivory Ink and Petrol Ink retain separate flower colours. Keep the secondary curl and leaf contours lighter than the principal stem, and do not enlarge the footprint to create apparent weight. Do not introduce a separate mid-width expertise grid. At 350 px and below the longest expertise pair separates to prevent overflow. The hero pause and the full-width rule before the project subsection share one central detail: a six-pixel antique-gold diamond inside a 20-pixel break in the line. The project rule extends to both content edges, while the hero uses only two short hairlines; from 1280 px upward, those hairlines may lengthen gradually to retain their visual presence in the wider field without changing the diamond or centre gap. The upper-left ornament pair remains preserved as approved source material. Preserve all approved and newly generated transparent source assets instead of reconstructing them from screenshots.
