...
- Full-section floral dividers use reinforced gold and emerald stems with theme-aware contrast, while their berry terminals and geometry stay delicate. On phones, scale the complete approved geometry slightly taller and wider so the compressed divider retains comparable visual weight; do not redraw or selectively enlarge individual motifs. Internal subsection and list rules remain visibly quieter.
...
The homepage hero uses `photo.jpg` with the approved lower-right ornament pair under `assets/hero/`: `quiet-atelier-lower-right.png` and `quiet-atelier-lower-right-dark.png`. From 760 px upward, the ornament sits to the lower right of the portrait-and-copy composition; the name is a larger Garamond identity line between the statement and sans-serif expertise. Below 760 px, use an identity-led lockup without the portrait or statement: the reinforced name appears first on exactly three lines, followed by reinforced expertise text. From 621 to 759 px the expertise forms a two-by-two visual grid and the enlarged ornament is integrated at the right. At 620 px and below the lockup is centred; the expertise forms a three-step inverted pyramid, and the enlarged ornament follows in normal document flow to complete its lower-right corner. At 350 px and below the longest expertise pair separates to prevent overflow. The hero pause and the full-width rule before the project subsection share one central detail: a six-pixel antique-gold diamond inside a 20-pixel break in the line. The project rule extends to both content edges, while the hero uses only two short hairlines. The upper-left ornament pair remains preserved as approved source material. Treat all five hero files as source assets and preserve their composition instead of redrawing them from screenshots.
