# Game of Worms scene review checklist

Updated: 2026-09-19. Use this file when Martyna asks which scene is next.

## Scope and counts

This is the current interaction and accessory follow-up pass, following the
earlier illustration revisions. It is not a new claim that all older scenes
have poor drawings or require animations.

- 31 active scenes, counted from the current accessory catalogue.
- 0 scenes remain to check on this pass's checklist.
- 31 have received the recent interaction pass, including Mauritius JU2909. Leave them closed unless Martyna explicitly reopens one.
- Mauritius final accessory and three-fruit revision was approved for deployment on 2026-09-16. This closes the requested follow-up; full-page capture and listening limitations remain documented.
- Mauritius follow-up approved for deployment, 2026-09-16: empty baskets and ten fruits outside, collected one per click. Final preview: `20260916-ground-fruit-4`.
- Oʻahu ECA789's workshop is live at `47b465c`. The final shared gift, mould removal and eating audio follow-up is approved for deployment.
- Kauaʻi QG130 is published at `e632c55`, verified on 2026-09-09.
- Saint-Benoît JU1373's final preview was approved for deployment on 2026-09-09.
- Ho Chi Minh City's approved revision is published and verified at `7e37b9a`.

"To check" means review once and decide **keep**, **small fix**, or **rework**.
Keeping an approved scene unchanged is a valid completed review. Absence of a
new animation is not a defect. No new concept is approved merely by appearing
in this checklist.

## Remaining queue

All 31 active scenes have now received this pass. There is no next scene on
this checklist. The final three were reviewed under Martyna's 2026-09-19
instruction to improve and deploy independently; completed scenes were not
reopened. See [final review](game-of-worms-reviews/final-scenes-20260919.md).

## Completed for this pass

These entries are based on the existing source, project records and the current
conversation, not a new browser tour. The first ten runtime revisions are in
the base checkout at `93d68c5`. Dois Rios was published separately at `8cbf580`,
with successful Pages run `34257605054`, public-file and live-browser checks.
Nambucca was published at `80310a1`, with successful Pages run `34264416595`
and public-file and live-browser verification on 2026-09-08.
Ho Chi Minh City was published at `7e37b9a`, with successful Pages run
`34275912526`, matching public text/audio and live-browser checks on 2026-09-08.
Completed here means closed for scene selection, not a claim of testing on every
browser or physical device.

| Species | Location | Status | Work already covered |
| --- | --- | --- | --- |
| elegans | Santeuil, France | Railway motion follow-up approved for deployment, 2026-09-22 | 18.2-second journey, larger-worm push and wave, male boarding/body pumping, gradual travel and braking, track-aligned wheels and rod, rail clacks plus recorded whistle. Original art and user transforms preserved. Organ remains separate. Preview `santeuil-ride-1`, port 8792. Deployment approved 2026-09-22. |
| elegans | Tenerife, Spain | Duet, shaker and snack follow-up approved for deployment, 2026-09-20 | User-selected follow-up: either timple leads a call-and-answer duet, shorter exchanges and shared ending. Hands and body bends follow scheduled sound; costumes track the body. Primary avocado now has a natural reach, bite and chew, with three portions eaten one per click and persistent depletion and timed recorded bite/chew sounds. Male bowl replaced by a handled gourd shaker and percussion activity. Guitar/costume art and visitor placements preserved. |
| elegans | Kauaʻi, Hawaiʻi | Vocal follow-up approved for deployment, preview `20260919-vocals-4` | Keep fitted headphones, two microphones and shared recorder. 2026-09-19 follow-up replaces whistles with sung call-and-response and matching tape playback. Responsive mouths/reels/meters/lamp. This is XZ1516, not QG130. |
| briggsae | Orsay, France · JU2518 | Steady-pose follow-up approved for deployment, 2026-09-19 | Preview `20260919-orsay-hold-1`: enter a distinct body curve, hold still while drawing, then release. Approved hand gestures retained; model crown, bag and notebook remain fixed. |
| briggsae | Salt Lake City, Utah - EG4181 | Approved for deployment, 2026-09-18 | Bubbles (independent paired kits), shared apricot picnic and folded blanket. Approved preview `20260918-picnic-3`, including the shorter quiet bites. Next: Orsay JU2518. |
| nigoni | Mahahual, Mexico · JU2617 | Done, deployment approved | Approved 2026-09-17, preview `20260917-mahahual-22`: shared sunscreen application, fitted separate bikini parts, recorded lotion foley, foreground umbrellas with independent tilt/height/size and moving ground shadows. Smaller canopy aligned with its shaft. Spectacles/background retained; bubbles deferred. |
| nigoni | Praslin, Seychelles · YR106 | Done, deployment approved | Approved 2026-09-17, preview `20260917-praslin-7`: both worms tuck fully into their shells, leaving tiny tail tips. Bell calls invite peeking and emergence. Original art, background and visitor transforms retained. |
| briggsae | Réunion Island · JU1375 | Done, deployment approved | Approved 2026-09-17, preview `20260917-reunion-7`: distinct vanilla arrangements, shared cane pressing and drinking, and two slower independent gecko walks that continue during worm activities and clear the labels. Background and saved transforms preserved. Validation limitations remain in the dossier. |
| tropicalis | New Taipei City, Taiwan · NIC1648 | Done, publication requested | 2026-09-16: larger pottery wheels, tailored canvas aprons with soft neck straps and bubble tea sipping. Hands leave vessel fronts visible; painting removed. Final revision `20260916-pottery-9`; user requested refinement and publication. Validation limitations remain in the dossier. |
| tropicalis | Guadeloupe · NIC203 | Done, deployment approved | Approved 2026-09-16, preview `20260916-gwoka-11`: twelve-beat drum phrases, smooth mid-body dancing, fitted layered hummingbird costumes and folded madras crowns. User positions/sizes and background preserved. See the Guadeloupe dossier for validation and capture/listening limitations. |
| nigoni | Mauritius · JU2909 | Done, final revision approved for deployment | Approved 2026-09-16: hand-played ravanne drums, clearer dodo grabbers and open woven baskets that begin empty. Ten outside fruits are collected one per click and remain in the baskets. Short labels: Ravanne drums, Fruit baskets, Dodo grabbers. Painting and user adjustment controls preserved. See the Mauritius dossier for QA and limitations. |
| nigoni | São Tomé · JU2484 | Done, deployment approved | Final contextual render approved 2026-09-13. Both worms board one snail, ride on its shell and climb off. Separate leaf reveal and recorded rustling. Flags removed, parasols and painting preserved. Approved general phoresy sentence added to Tiny surprise. See the JU2484 dossier for evidence and QA. |
| tropicalis | Pohnpei, Micronesia · QG4739 | Birdwatching revision approved for deployment | 2026-09-22: user approved Birdwatching, Bird calls and Camouflage capes. Replaced the leaf sled with a calling, hopping lorikeet and short flight, binocular lift and male scope pose. Manual magnification and user transforms preserved. Labels and contextual preview approved for deployment 2026-09-22. |
| tropicalis | Queensland, Australia · QG2904 | Done, deployment approved | Approved 2026-09-13. Photography fix and deployment requested 2026-09-19: lift waits at canopy height for repeat photos, tap basket to descend; tap tolerance and moving basket hit target corrected. Shared canopy lift, independently usable cameras and adjustable harnesses during the ride, developing photobomb print and three recorded sound channels. Approved drawings and visitor adjustments preserved. Capture and listening limitations remain documented. |
| nigoni | Lombok, Indonesia · HPT26 | Forest follow-up approved for deployment | 2026-09-21: user chose Leaf fans and Butterflies alongside fig hide-and-seek. Revised fig skin and seeded interiors; held fans wave with quiet leaf rustle; blue butterflies fly independently on broad paths and return on a second click. Fig finish flash fixed; leaves refined. Pool and goggles retired. Background and visitor fig transforms preserved. Final forest-4 and compact-1 approved for deployment on 2026-09-21. English-only labels: Leaf fans, Figs, Butterflies. |
| tropicalis | Oʻahu, Hawaiʻi · ECA789 | Done, follow-up approved for deployment | Workshop deployed at `47b465c`. One large shared gift, mould removal, smaller bike hit area and recorded eating cues approved for deployment. Chocolate bike and tasting artwork and sizes preserved. Capture and listening limitations remain in the review notes. |
| briggsae | Kauaʻi, Hawaiʻi · QG130 | Done | Forest bath with ginger squeezed into the male's bowl, jug/dipper rinsing, fitted towels and recorded water/cloth sounds. Original forest painting retained. Published at `e632c55`, with successful Pages run `34347245958` and matching public files. Full-page capture and manual listening limitations remain documented. |
| tropicalis | Saint-Benoît, Réunion · JU1373 | Follow-up deployment requested, 2026-09-23 | Lychees, fitted coats/rain and shared flower arranging retained. Added planted body movement and reaching; the finished bouquet stays through other prop actions and resizing until another flower tap. See PROJECT_STATUS.md for QA and capture limitations. |
| inopinata | Ishigaki, Japan | Done | Wing flutter/flight with sound, figs, visors and baskets. |
| briggsae | Ahmedabad, India · AF16 | Done | Kites, hands, digging, costumes, sound and mobile pair spacing. |
| elegans | Bristol N2, England | Follow-up deployment requested, 2026-09-23 | Plates lift with supporting hands, mouth pumping and one consumed colony per tap. Separate portions per worm and explicit refill after an empty plate. Fitted coats, goggles and cryopack flight retained. |
| elegans | Edinburgh, Scotland | Done | Kilt fit, usable telescopes and playable bagpipes. |
| elegans | Australian Capital Territory | Done | Coffee and biscuits, faster cockatoo theft, napkins and sound. |
| elegans | Claremont, California · ECA250 | Books-7 approved for deployment, 2026-09-20 | Two reader-facing picture stories with clearer pages and individual reading/page turns. Male spacing and hand layering refined; each reader keeps its own pose and sounds. Visitor placements preserved. |
| elegans | Araucanía, Chile | Follow-up deployment requested, 2026-09-23 | Lower wheelbarrow/fork, grounded tipping, body bends toward the handles, slower boarding before rolling, travel-matched wheel and moving shadow. Food, mate, sounds and compost gem preserved. |
| nigoni | Trivandrum, Kerala · JU1325 | Sample handoff implementation and deployment requested, 2026-09-22 | Female opens/tips her tube; male receives the leaf in a distinct examination dish, raises it and his loupe, and watches a tiny moving specimen. Planted body bends and supporting hands. Shower, saved transforms, live magnification and female tube gem hook preserved. English-only Samples label. Ahmedabad left unchanged by request. |
| wallacei | Sanda, Bali · JU1873 | Gong duet approved for deployment, 2026-09-20 | Preview gong-duet-2: shared 15-strike exchange with gripped mallets, body bends and three recorded gong tones. Exact return to visitor positions; cacao opening/eating, wraps and approved art preserved. Publication requested by Martyna. |
| tropicalis | Barro Colorado Island, Panama | Shared mixing follow-up approved for deployment, 2026-09-20 | Preview mixing-5: one sequence for adding flowers, blending, pouring into the male bowl and spooning into the dish. Recorded motor, spoon and pour; fitted headpiece tracking and natural reaching. Existing artwork, flower actions and leaf-cutting ants preserved. Martyna approved mixing-5 and requested publication. |
| briggsae | Dois Rios, Ilha Grande, Brazil · EG5612 | Follow-up deployment requested, 2026-09-23 | Existing jackfruit, instruments, neckerchiefs and sounds retained. Added planted body movement during the duet and sticky-fruit release/recoil. Fixed duplicate primary performance actor and face restoration. See PROJECT_STATUS.md for QA and capture limitations. |
| briggsae | Nambucca Heads, New South Wales · QG2814 | Done | Flower presses, rock painting and fitted painting aprons approved and deployed at `80310a1`. Full-page capture and manual listening limitations remain in PROJECT_STATUS.md. 2026-09-21 follow-up implemented for requested deployment: body bends toward craft work, fitted aprons follow, completed stone lifts for inspection and returns. |
| nigoni | Ho Chi Minh City · JU4356 | Done | Shared scooter, starfruit cutting/tasting and phin/iced coffee approved and deployed at `7e37b9a`. Recorded audio, one activity at a time and preserved accessory sizing. Capture and manual-test limitations remain in the location dossier. |

## How to keep the list useful

Small shared-map issue noted during Nambucca QA: at the normal narrow browser
width, a pointer click targeted at Nambucca selected nearby Queensland. Keyboard
selection reached the correct record. Recheck these two hit areas separately
from the scene artwork. No map change was made in this preview.

1. Open this checklist first. Inspect only the next selected scene and its
   relevant files. Do not tour the whole atlas again.
2. Record the review date, specific finding and decision. Move a scene out of
   the queue when it is reviewed, even if the decision is to keep it unchanged.
3. Keep "implemented locally", "approved", "committed", and "live verified"
   separate. Add a commit/reference when publication actually happens.
4. Reopen a closed scene only when Martyna reports a problem, a relevant file
   changes, or a shared rendering/interaction regression could affect it.
5. A shared-system change needs targeted regression checks on affected scenes,
   not a fresh judgement of every landscape and accessory.
6. Reconcile the inventory only when locations are added or removed. The exact
   species/location pairs in these tables must match the current catalogue once
   each, so the two Kauaʻi and two Réunion scenes cannot be confused.

Sources for this ledger: current `game-of-worms/accessory-designs.js`, current
`PROJECT_STATUS.md`, `docs/game-of-worms-art-catalogue.md`,
`docs/game-of-worms-six-location-refinement.md` and the conversation's review
decisions. The catalogue's old descriptive rows can lag its production overrides.
Use the current code for the selected scene instead of reusing obsolete props.
