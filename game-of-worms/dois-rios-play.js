...
export function fruitFrame(ms,male,reduced=false){
  ...
  const recoil=Math.sin(Math.PI*clamp((ms-releaseAt)/650))*back;
  return {done:ms>=2900,lift,lean:-5*load*(1-release),stretch:ms<releaseAt?load:0,release:ms>=releaseAt,recoil};
}
...
export function createDoisRiosPlay(habitat){
  ...
  function cancel(){
    ...
    active.players?.forEach(p=>p.rig.restore());active.effects.remove();delete habitat.dataset.doisRiosAction;active=null;
  }
  function start(piece){
    ...
    const pieces=kind==='music'?[...habitat.querySelectorAll(`.accessory-piece[data-accessory-family="${MUSIC}"]`)].filter(n=>!n.closest('[hidden]')):[piece];
    ...
    const players=pieces.map(p=>{
      ...
      const rig=workingBody(habitat,p.dataset.wormPart,['eg5612-chita-neckerchiefs']);
      ...
      return {piece:p,male,art,body,node,latex,thread,arms,rig};
    });
    run.players=players;
    ...
    function tick(now){
      ...
      for(const p of players){
        ...
        let targets;const envelope=ease(ms/350)*(1-ease((ms-(kind==='music'?3500:2250))/600));
        if(!reduced.matches){
          const sway=Math.sin((ms-35)/1000*Math.PI*2);
          p.rig.set(kind==='music'?(p.male?5:-7)*sway*envelope:s.lean*2.2+(s.recoil||0)*7,
            kind==='music'?(p.male?5:3)*s.hit*envelope:4*s.stretch-5*(s.recoil||0),
            kind==='music'?(p.male?-8:11)*sway*envelope:-9*s.stretch+5*(s.recoil||0));
        }
        if(kind==='fruit'){
        ...
        p.arms.forEach((a,i)=>{
          const from=p.rig.point(p.male?i?267:245:i?240:299,p.male?128:i?138:113),to=targets[i],dx=to.x-from.x,dy=to.y-from.y;
          ...
          a.arm.setAttribute('opacity',reduced.matches?1:envelope);
        });
      }
