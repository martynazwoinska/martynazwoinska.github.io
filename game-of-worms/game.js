...
import { auditAccessoryCatalogue, auditAccessoryPairGeometry, renderLocationAccessories } from "./accessory-designs.js?v=20260918-picnic-2";
...
import { createPanamaPlay } from "./panama-play.js?v=20260914-ant-visitors-1";
...
const species = [
  ...
  },
  {
    ...
    locations: [
      ...
      { name: "Nambucca Heads, New South Wales · QG2814", sceneLabel: "Rotting flowers · Nambucca Heads · Australia", coordinates: [153.0090333, -30.6445167], source: "CaeNDR QG2814 isotype record", style: "rainforest", strain: "QG2814", history: "One sample of rotting flowers from a Nambucca Heads rainforest garden produced two laboratory strains." }
    ]
  },
  {
    id: "elegans",
  ...
  },
  {
    ...
    locations: [
      ...
      { name: "Oʻahu, Hawaiʻi · ECA789", sceneLabel: "Rotting flower · Oʻahu · United States", coordinates: [-157.799786, 21.336056], source: "CaeNDR ECA789 isotype record", style: "rainforest", strain: "ECA789", history: "This worm came from a rotting flower along Mānoa Falls Trail. Near the collection site, Mānoa Chocolate makes chocolate using locally grown cacao. The factory once used a child’s bicycle to power a machine that separated cacao nibs from their shells." },
...
const araucaniaPlay = createAraucaniaPlay(els.habitat,updateAccessoryLabelVisibility);
...
const mauritiusPlay = createMauritiusPlay(els.habitat, piece => {
  toggleAccessory("local-wrap", true);
...
const oahuBike = createOahuChocolate(els.habitat, () => { refreshAccessoryPieceControls(); queueAccessoryConstraints(); });
...
const nambuccaPlay = createNambuccaPlay(els.habitat, () => {
  refreshAccessoryPieceControls();
  queueAccessoryConstraints();
});
function renderSpecies(item, place) {
  ...
  saltLakePlay.cancel(); mahahualPlay.cancel(); praslinPlay.cancel(); ju1375Play.cancel(); taipeiPlay.cancel(); guadeloupePlay.cancel(); mauritiusPlay.cancel(); panamaPlay.cancel();
  doisRiosPlay.cancel();
  nambuccaPlay.cancel();
  oahuBike.cancel();
  hcmcPlay.cancel();
...
function addAccessoryHitTarget(piece) {
  ...
  if (piece.dataset.accessoryFamily === "guadeloupe-hummingbird-costume") {
    // Pad the vest, not the empty rectangle between wings and beak.
    const rig = piece.querySelector('[data-gua-costume-rig]');
    const centre = new DOMPoint(218,152).matrixTransform(matrix.inverse().multiply(rig.getScreenCTM()));
    const hitTarget = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    const width=44/scaleX,height=44/scaleY;
    for(const [key,value] of Object.entries({class:'accessory-hit-target',x:centre.x-width/2,y:centre.y-height/2,width,height,rx:5,'aria-hidden':'true'}))hitTarget.setAttribute(key,String(value));
    piece.prepend(hitTarget);return;
  }
...
function finishAccessoryDrag(event) {
  ...
  if (event.type === "pointercancel") { saltLakePlay.cancel(); mahahualPlay.cancel(); praslinPlay.cancel(); ju1375Play.cancel(); taipeiPlay.cancel(); }
...
function moveActiveAccessoryPointer(event) {
  ...
  if ((saltLakePlay.handles(piece) || mahahualPlay.handles(piece) || praslinPlay.handles(piece) || ju1375Play.handles(piece) || taipeiPlay.handles(piece)) && Math.hypot(event.clientX-activeAccessoryDrag.startClientPoint.x,event.clientY-activeAccessoryDrag.startClientPoint.y)>6) {
    saltLakePlay.cancel(); mahahualPlay.cancel(); praslinPlay.cancel(); ju1375Play.cancel(); taipeiPlay.cancel();
  }
  ...
  if(lombokPlay.active && Math.hypot(event.clientX-activeAccessoryDrag.startClientPoint.x,event.clientY-activeAccessoryDrag.startClientPoint.y)>2) {
    lombokPlay.cancel();activeAccessoryDrag.startBounds=accessoryPieceBounds(id,wormPart);
  }
  if(oahuBike.active && Math.hypot(event.clientX-activeAccessoryDrag.startClientPoint.x,event.clientY-activeAccessoryDrag.startClientPoint.y)>2) {
    oahuBike.cancel();activeAccessoryDrag.startBounds=accessoryPieceBounds(id,wormPart);
  }
  ...
  if (activeAccessoryDrag.pinch && activeAccessoryDrag.pointers.size === 2) {
    ...
    if (Math.abs(distance - activeAccessoryDrag.pinch.startDistance) > 1) {
      if (!activeAccessoryDrag.moved) { saltLakePlay.cancel(); mahahualPlay.cancel(); praslinPlay.cancel(); ju1375Play.cancel(); taipeiPlay.cancel(); }
  ...
  if (!activeAccessoryDrag.canopyPoint && startBounds?.width && startBounds.height) {
  ...
  const point=activeAccessoryDrag.canopyPoint&&canopyPlay.pointerPoint(piece,event.clientX,event.clientY);
  const delta = point
    ? {x:point.x-activeAccessoryDrag.canopyPoint.x,y:point.y-activeAccessoryDrag.canopyPoint.y}
    : screenDeltaToAccessorySpace(piece, screenDeltaX, screenDeltaY);
  moveAccessory(id, wormPart, {
...
function wireAccessoryPieces() {
  document.querySelectorAll(".accessory-piece[data-worm-part]").forEach(piece => {
    ...
    piece.addEventListener("keydown", event => {
      if ((event.key === "Enter" || event.key === " ") && (saltLakePlay.handles(piece) || mahahualPlay.handles(piece)) && !drawingEnabled && activeWardrobe().has(id)) {
        event.preventDefault(); if (!event.repeat) { saltLakePlay.start(piece); mahahualPlay.start(piece); } return;
      }
      ...
      if (["Escape", "Home", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) { saltLakePlay.cancel(); mahahualPlay.cancel(); praslinPlay.cancel(); ju1375Play.cancel(); taipeiPlay.cancel(); }
      ...
      if ((event.key === "Enter" || event.key === " ") && araucaniaPlay.handles(piece)) {
        event.preventDefault();if(!event.repeat)araucaniaPlay.start(piece);return;
      }
      if(event.key === "Home")araucaniaPlay.reset(piece);
      else if (["Escape", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key))araucaniaPlay.cancel();
      ...
      if ((event.key === "Enter" || event.key === " ") && lombokPlay.handles(piece)) {
        event.preventDefault(); if (!event.repeat) lombokPlay.start(piece); return;
      }
      ...
      if ((event.key === "Enter" || event.key === " ") && oahuBike.handles(piece)) {
        event.preventDefault(); if (!event.repeat) oahuBike.start(piece); return;
      }
      if (event.key === "Home") oahuBike.reset();
      else if (["Escape", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) oahuBike.cancel();
      if ((event.key === "Enter" || event.key === " ") && hcmcPlay.handles(piece)) {
        event.preventDefault(); if (!event.repeat) hcmcPlay.start(piece); return;
      }
      if (["Escape", "Home", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) hcmcPlay.cancel();
      if ((event.key === "Enter" || event.key === " ") && nambuccaPlay.handles(piece)) {
        event.preventDefault(); if(!event.repeat)nambuccaPlay.start(piece); return;
      }
      if (event.key === "Home") nambuccaPlay.reset(piece);
      else if (["Escape", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) nambuccaPlay.cancel();
      if ((event.key === "Enter" || event.key === " ") && panamaPlay.handles(piece)) {
        event.preventDefault(); if(!event.repeat)panamaPlay.start(piece); return;
      }
      if (event.key === "Home") panamaPlay.reset(piece);
      else if (["Escape", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) panamaPlay.cancel();
