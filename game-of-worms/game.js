import { createMauritiusPlay } from "./mauritius-play.js?v=20260915-fruit-drums-1";
...
import { auditAccessoryCatalogue, auditAccessoryPairGeometry, renderLocationAccessories } from "./accessory-designs.js?v=20260914-ant-visitors-1";
...
function renderSpecies(item, place) {
  ...
  mauritiusPlay.cancel(); panamaPlay.cancel();
  doisRiosPlay.cancel();
  nambuccaPlay.cancel();
  oahuBike.cancel();
  hcmcPlay.cancel();
...
function finishAccessoryDrag(event) {
  ...
  if (moved || !ahmedabadHands.handles(piece) && !panamaPlay.handlesLeaf(piece) && !mauritiusPlay.handles(piece)) moveAccessory(id, wormPart, accessoryPosition(id, wormPart), piece);
  if (moved) {
