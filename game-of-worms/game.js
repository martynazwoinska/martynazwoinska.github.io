import { createFinalScenes } from "./final-scenes-play.js?v=20260922-railway-1";
...
import { createPohnpeiPlay } from "./pohnpei-play.js?v=20260922-birdwatch-1";
...
import { createN2Feeding } from "./n2-feeding.js?v=20260923-bristol-bites-1";
...
import {createTreasureHunt} from './treasure-hunt.js?v=20260923-puzzle-caption-1';
...
els.accessorySizeSlider.addEventListener("input", event => {
  ...
  hcmcPlay.cancel();
  reunionPlay.cancel();
  if (!trivandrumWatering.preservesSample(target&&visibleAccessoryPieces(target.id,target.wormPart)[0])) trivandrumWatering.cancel();
...
function finishAccessoryDrag(event) {
  ...
  if (moved || !trivandrumWatering.preservesSample(piece) && !claremontPlay.handles(piece) && !canopyPlay.handles(piece) && !ahmedabadHands.handles(piece) && !panamaPlay.handlesLeaf(piece) && !mauritiusPlay.handles(piece) && !guadeloupePlay.handles(piece) && !taipeiPlay.handles(piece) && !ju1375Play.handles(piece) && !(finalScenes.handles(piece) || orsayPlay.handles(piece) || saltLakePlay.handles(piece)) && !mahahualPlay.handles(piece) && !praslinPlay.handles(piece)) moveAccessory(id, wormPart, accessoryPosition(id, wormPart), piece);
  if (moved) {
...
function moveActiveAccessoryPointer(event) {
  ...
  if (trivandrumWatering.preservesSample(piece) && !activeAccessoryDrag.moved && !activeAccessoryDrag.pinch && Math.hypot(event.clientX-activeAccessoryDrag.startClientPoint.x,event.clientY-activeAccessoryDrag.startClientPoint.y)<=6) return;
...
function wireAccessoryPieces() {
  document.querySelectorAll(".accessory-piece[data-worm-part]").forEach(piece => {
    ...
    piece.addEventListener("pointerdown", event => {
      ...
      hcmcPlay.cancel();
      reunionPlay.cancel();
      if (!trivandrumWatering.preservesSample(piece)) trivandrumWatering.cancel();
    ...
    piece.addEventListener("keydown", event => {
      ...
      if (["Escape", "Home"].includes(event.key) || !trivandrumWatering.preservesSample(piece) && ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "+", "=", "-", "_"].includes(event.key)) trivandrumWatering.cancel();
