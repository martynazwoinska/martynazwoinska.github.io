...
// Leave room between the male's face/shoulders and the far edge of his book.
export function companionReadingPose(dx,dy,lean,sleep){
 return {x:dx*lean-4*sleep,y:Math.min(20,dy)*lean+12*sleep,angle:5*lean+9*sleep};
}
export function createReaders(habitat,previous=null,smallBook=false){
 ...
 for(const part of [smallBook?'companion':'primary']){
  ...
  const book=habitat.querySelector(`.accessory-piece[data-accessory-family="eca250-bookworm-book"][data-worm-part="${part}"] [data-book-plane]`);
  const target=book?at(base.inverse().multiply(relative(root,book)),-9,0):{x:290,y:200};
  actors.push({part,male,body,base,children,paths,face,eyes,glasses,glassesWrap,glassesParent,glassesStyles,
   dx:Math.max(-65,Math.min(30,target.x-329)),dy:Math.max(-30,Math.min(92,target.y-110)),pose:{x:0,y:0,angle:0}});
 }
 const layer=add(root,'g',{'data-reading-hands':smallBook?'companion':'primary','pointer-events':'none','aria-hidden':'true'});
 const armLayer=smallBook?add(root,'g',{'data-reading-arms':'companion','pointer-events':'none','aria-hidden':'true'}):layer;
 if(smallBook)root.prepend(armLayer);
 const hands=[0,1].map(()=>({back:add(armLayer,'path',{fill:'none',stroke:'var(--worm-deep)','stroke-width':5.5,'stroke-linecap':'round'}),front:add(armLayer,'path',{fill:'none',stroke:'var(--worm-color)','stroke-width':3.5,'stroke-linecap':'round'}),tip:add(layer,'ellipse',{rx:3.6,ry:2.8,fill:'var(--worm-color)',stroke:'var(--worm-deep)','stroke-width':.7})}));
 let frame=readingFrame(0,previous);
 function point(a,x,y){const q=readingPoint(x,y,a.pose);return at(a.base,q.x,q.y);}
 function paint(f,turning=null,book=null,small=smallBook){
  frame=f;
  for(const a of actors){const v=f[a.part];a.pose=a.male?companionReadingPose(a.dx,a.dy,v.lean,v.sleep):{x:a.dx*v.lean+v.scan*35,y:a.dy*v.lean,angle:15*v.lean};
  ...
  for(let i=0;i<2;i++){const h=hands[i],from=small?point(owner,i?310:280,i?93:107):point(owner,i?220:251,i?161:136),node=i?book:turning;
   const target=node?at(relative(root,node),i?55:small?-203:-150,small?-62:35):from,amount=f.hand;
   const end={x:mix(from.x,target.x,amount),y:mix(from.y,target.y,amount)};
   for(const n of [h.back,h.front]){n.setAttribute('d',`M${from.x} ${from.y}Q${(from.x+end.x)/2} ${small?Math.min(from.y,end.y)-9:Math.max(from.y,end.y)+8} ${end.x} ${end.y}`);n.setAttribute('stroke-width',small?(n===h.back?3.2:1.8):(n===h.back?5.5:3.5));}
